	...
	if request.method == 'POST':
		...
		try:
			...
			if request.POST['post'] == 'Patient':
				...
				new = Doctor(phone=request.POST['phone'],name=request.POST['name'],email=request.POST['email'],username=user)	
				new.save()	
				return render(request,'register.html')
...
def create_appointment_patient(request , user):
	status = False
	if request.user:
		status = request.user
	...
	patient_names = Patient.objects.filter(name = request.user)
	doctor_names = Doctor.objects.all()	

	return render(request , 'patient_appointment.html' , {'user':user, "status": status , "patient_names" : patient_names , 
		"doctor_names" : doctor_names })
...
def doctor_prescription(request):
	...
	doctor = Doctor.objects.get(username=user_id)
	print(doctor)
	pers   = Prescription2.objects.filter(doctor = doctor)
	print(len(pers))
	for i in pers:
		print(i.patient)
...
def create_prescription(request):
	...
	if request.method == 'POST':
		...
		user_id = User.objects.get(username=request.user)
		doctor = Doctor.objects.get(username=user_id)
		new_prescrition = Prescription2(symptoms = request.POST['symptoms'] , prescription = request.POST['prescription'] , patient = appointment.patientid , doctor = doctor , appointment = appointment)
...
def patient_invoice(request):
	...
	data = Invoice.objects.filter(patient = p)
	print(data)
